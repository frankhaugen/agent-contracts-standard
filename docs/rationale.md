...
- **Strict dogfood:** The reference repository can carry a full `.ai/` maintainer contract and automated checks (`scripts/Verify-AcsRepo.ps1`) so the standard’s publisher eats its own cooking at a higher bar than the Section 9 minimum alone.
- **Spec vs publication:** The normative standard is **only** what appears in `spec/ACS-v0.1.md` Sections 1–10; how this repo lays out `templates/`, `examples/`, and docs is **publisher material**, not part of the ACS text ([`docs/publisher-layout-and-templates.md`](publisher-layout-and-templates.md)).
