...
ACS defines **contract taxonomy**, **required paths** under `.ai/`, **file roles**, **resolution order**, and **vendor neutrality** for repository contracts (Section 6). It does **not** define runtimes or orchestration. See Section 2 in the spec.
...
- Vendor neutrality per spec Section 6 (no **required** dependence on vendor-specific agent artifacts; **Forbidden paths** such as any `.cursor`/`.copilot`/`.claude` tree and GitHub Copilot paths under `.github/` **MUST NOT** exist—see spec).
- Everything is discoverable starting from `AGENTS.md`.

Compliance checklist: [Section 9](spec/ACS-v0.1.md#9-compliance). **Normative references** (BCP 14, AGENTS.md, MCP, Agent Skills): [Section 10](spec/ACS-v0.1.md#10-normative-references).
