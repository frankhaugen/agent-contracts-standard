...
# spec Section 6.3 Forbidden paths — absolute prohibition (version-controlled tree)
...
foreach ($name in $ForbiddenDotNames) {
    ...
    foreach ($h in $hits) {
        ...
        [void]$Errors.Add("Forbidden name MUST NOT exist in tree (spec ACS Section 6.3): $rel")
    }
}
...
foreach ($rel in $ForbiddenGithubRel) {
    ...
    if (Test-Path -LiteralPath $p) {
        [void]$Errors.Add("Forbidden path MUST NOT exist (spec ACS Section 6.3): $rel")
    }
}
...
if ($agents -notmatch '\.ai/index\.md') {
    [void]$Errors.Add('AGENTS.md MUST reference `.ai/index.md` for ACS discoverability (Section 6 / 9).')
}
...
if ($positions -contains -1) {
    ...
    for ($i = 1; $i -lt $positions.Count; $i++) {
        if ($positions[$i] -le $positions[$i - 1]) {
            [void]$Errors.Add('.ai/index.md section order MUST follow ACS Section 8: Instructions, Policies, Skills, Commands, Context.')
            break
        }
    }
}
